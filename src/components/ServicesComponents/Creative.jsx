import { Link } from 'react-router-dom';

const Creative = () => {
    return (
        <section className="p-32">
            <div className="grid grid-cols-2 justify-items-center items-center container mx-auto">
                <div>
                    <h1 className="text-3xl font-semibold text-blue-500">Creative Ideas</h1>
                    <p className="text-sm mt-2 text-justify">Think, imagine, create, improve business marketing strategies with ideas that comes out-of-the-box.</p>
                    <button className="p-2 text-sm bg-blue-500 rounded-sm text-gray-100 mt-2 hover:bg-transparent hover:border hover:border-blue-500 hover:text-blue-500 transition">
                        <Link to="/contact">Contact Us</Link>
                    </button>
                </div>
            </div>
        </section>
    )
}

export default Creative;