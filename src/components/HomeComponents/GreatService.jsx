        ...
        <section className="p-32 h-[500px] grid grid-cols-2 container mx-auto">
            <div>
                <div className="text-gray-700">
                    ...
                    <p className="text-justify text-sm mt-2">CAZA Technology Solutions Inc. offer the full spectrum of services to help organizations work better.
                ...
                <div className="mt-5">
                    ...
                    { buttons?.map(button => (
                        <button key={button.id} onClick={() => setCurrentBtn(button.title)} className={`${ button.title === currentBtn ? 'bg-blue-500 text-white' : '' } border w-full hover:text-white hover:bg-blue-500 transition text-blue-500 border-gray-300 p-2 cursor-pointer text-sm`}>{button.title}</button>
                    )) }
                    </div>
                    ...
                    <AnimatePresence>
                        ...
                        className="text-sm text-gray-700 mt-3 transition">{ buttons.filter(button => button.title === currentBtn).map(button => button.description) }</motion.p>
                    </AnimatePresence>
                </div>
            </div>
        </section>
